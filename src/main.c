...
int main(int argc, char **argv) {
    ...
    fprintf(f_variational, "alpha,T,T_std,T_lap,T_lap_std,T_grad,T_grad_std,VHO,VHO_std,VLJ,VLJ_std,E,E_std\n");
    ...
    for (double alpha = alpha_start; alpha <= alpha_end; alpha += alpha_step) {
        ...
        fprintf(f_variational, "%.2f,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e,%.10e\n", alpha, T_avg, T_std/sqrt(n_steps), T_lap_avg, T_lap_std/sqrt(n_steps), T_grad_avg, T_grad_std/sqrt(n_steps), VHO_avg, VHO_std/sqrt(n_steps), VLJ_avg, VLJ_std/sqrt(n_steps), E_avg, E_std/sqrt(n_steps));
